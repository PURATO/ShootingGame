...
public class ShootingView extends SurfaceView
    ...
    public void run() {
        while(thread != null) {
            ...
            g.setTextSize(30);
            g.setTypeface(segmentFont);
            g.drawText(num2str(score,6), 60+g.getOriginX(), 60-(int)g.getFontMetrics().ascent);
            g.setTypeface(nicoFont);
            g.drawText("SCORE ", 30+g.getOriginX(), 20-(int)g.getFontMetrics().ascent);
            */
            drawScore();
            drawFPS((int)offsetTime);
            g.unlock();
    ...
    public void drawFPS(int fps) {
        ...
        g.setTextSize(30);
        g.setTypeface(segmentFont);
        g.drawText(num2str(fps,2), 300+g.getOriginX(), 60-(int)g.getFontMetrics().ascent);
        g.setTypeface(nicoFont);
